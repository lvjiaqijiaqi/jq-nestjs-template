...
export default registerAs('app', () => ({
  ...
  cors: {
    ...
    allowedHeaders: (process.env.CORS_ALLOWED_HEADERS || 'Content-Type,Authorization,X-Requested-With,X-API-Version,X-Request-Id').split(','),
    exposedHeaders: (process.env.CORS_EXPOSED_HEADERS || 'X-Total-Count,X-Request-Id,X-API-Version').split(','),
    maxAge: parseInt(process.env.CORS_MAX_AGE || '86400', 10), // 24小时
  },

  // 文档配置
  docs: {
    enabled: process.env.DOCS_ENABLED !== 'false' && process.env.NODE_ENV !== 'production',
    path: process.env.DOCS_PATH || 'docs',
    title: process.env.DOCS_TITLE || 'NestJS 企业级样板 API',
    description: process.env.DOCS_DESCRIPTION || '企业级 NestJS 样板项目 API 文档',
  ...
  features: {
    ...
    // 是否启用详细错误信息（仅开发环境）
    detailedErrors: process.env.FEATURE_DETAILED_ERRORS === 'true' || process.env.NODE_ENV === 'development',
    // 是否启用数据库查询日志
    queryLogging: process.env.FEATURE_QUERY_LOGGING === 'true' || process.env.NODE_ENV === 'development',
  },

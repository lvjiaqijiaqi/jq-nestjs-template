...
export default registerAs('queue', () => ({
  ...
  queues: {
    ...
    notification: {
      ...
      defaultJobOptions: {
        ...
        backoff: {
          type: 'exponential',
          delay: parseInt(process.env.QUEUE_NOTIFICATION_BACKOFF_DELAY || '1000', 10),
        },
      },
    },
  ...
  concurrency: {
    ...
    notification: parseInt(process.env.QUEUE_NOTIFICATION_CONCURRENCY || '10', 10),
  ...
  global: {
    // 默认作业选项
    defaultJobOptions: {
      removeOnComplete: parseInt(process.env.QUEUE_GLOBAL_REMOVE_ON_COMPLETE || '100', 10),
      removeOnFail: parseInt(process.env.QUEUE_GLOBAL_REMOVE_ON_FAIL || '50', 10),
    },
    
    // 队列设置
    settings: {
      stalledInterval: parseInt(process.env.QUEUE_STALLED_INTERVAL || '30000', 10), // 30秒

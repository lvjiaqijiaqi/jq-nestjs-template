...
export class EmailProcessor {
  ...
  async handleSendBulkEmail(job: Job<{ emails: EmailJobData[] }>) {
    ...
    try {
      this.logger.log(
        `Processing bulk email job ${job.id}: ${emails.length} emails`,
      );
      ...
      this.logger.log(
        `Bulk email job ${job.id} completed: ${results.filter((r) => r.success).length}/${emails.length} successful`,
      );
      return { results };
    } catch (error) {
